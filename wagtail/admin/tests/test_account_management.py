...
class TestAccountSection(
    ...
    def test_doesnt_render_locale_panel_when_only_one_timezone_and_one_locale_permitted(
        self,
    ):
        response = self.client.get(reverse("wagtailadmin_account"))
...
class TestAccountUploadAvatar(WagtailTestUtils, TestCase, TestAccountSectionUtilsMixin):
    ...
    def test_set_custom_avatar_stores_and_get_custom_avatar(self):
        response = self.post_form(
            {
                "avatar-avatar": SimpleUploadedFile(
                    "other.png", self.other_avatar.file.getvalue()
                )
            }
        )
        # Check that the user was redirected to the account page
...
class TestPasswordReset(WagtailTestUtils, TestCase):
    ...
    def test_password_reset_confirm_view_post(self):
        ...
        self.assertTrue(
            get_user_model()
            .objects.get(email="test@email.com")
    ...
    def test_password_reset_confirm_view_post_password_mismatch(self):
        ...
        self.assertIn(
            "The two password fields didn’t match.",
            response.context["form"].errors["new_password2"],
        )
